...
function gas_render_pdf_or_page_field( $args ) {
	...
	// PDF Upload Button
	echo '<div class="gas-pdf-upload-field pdf-upload" style="' . ( $selected_type !== 'pdf' ? 'display:none;' : '' ) . 'margin-bottom: 10px;">';
	if ( $pdf_id ) {
	...
	// Pages Dropdown
	echo '<div class="gas-page-select-field"' . ( $selected_type !== 'page' ? ' style="display:none; margin-bottom: 10px;"' : ' style="margin-bottom: 10px;"' ) . '>';
	wp_dropdown_pages(
		array(
			'name'             => esc_attr( $field_id ) . '[page]',
			'selected'         => $field_value['page'] ?? 0,
...
function gas_sanitize_pdf_or_page( $input ) {
	$output = array();
	if ( isset( $input['type'] ) && $input['type'] === 'pdf' && ! empty( $input['pdf'] ) ) {
		// Sanitize URL for the PDF
		$output['type'] = 'pdf';
		$output['pdf']  = esc_url_raw( $input['pdf'] );
	} elseif ( isset( $input['type'] ) && $input['type'] === 'page' && ! empty( $input['page'] ) ) {
